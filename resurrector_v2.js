...
function setUnicornStatus(unicornId) {
    log('upd status for:', unicornId);
    br_contract.methods.offers(unicornId).call({from: fromAddress})
        .then(offer=>{
            if (!offer.exists) {
    ...
    br_contract.methods.hybridizations(unicornId).call({from: fromAddress})
        .then(hybr=>{
            if (!hybr.exists) {
                updateStatus(unicornId, {
                    blockchain_id: unicornId,
                ...
                transaction_semaphore.take(() => {
                    web3.eth.sendTransaction(
                        ...
                        .on('transactionHash', function (hash) {
                            ...
                            log('set gene', unicornId, 'tx hash:', nonce, hash);
                            nonce++;
                            transaction_semaphore.leave();
